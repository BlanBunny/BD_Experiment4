...
public class code
{
    ...
    public static class TokenizerMapper extends Mapper <Object, Text, Text, IntWritable>{
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
            //String line = (caseSensitive)? //2.0
            //        value.toString() : value.toString().toLowerCase();   //2.0 不区分大小写


            StringTokenizer itr = new StringTokenizer(value.toString());         //用StringTokenizer将字符串拆成单词
            //StringTokenizer itr = new StringTokenizer(line);  //2.0
    ...
    public static class AutoReducer extends Reducer<IntWritable,Text,Text, NullWritable>{
        ...
        public void reduce(IntWritable key,Iterable<Text> values,Context context) throws IOException,InterruptedException{
            for(Text v:values)
            {
                ...
                String res="No "+count+"- merchant_id: "+v_string+", times: "+key.toString();
    ...
    public static void main(String[] args) throws Exception{
        ...
        // 只有当job任务成功执行完成以后才开始sortJob，参数true表明打印verbose信息
        {
            ...
            sortJob.setJarByClass(code.class);
